...
//button variables
  int paFlag, paFlag1, P4 = 0;
  unsigned long pinActivated = 0;
  unsigned long buttonDelay = 100;

void pinAct() {

  if( digitalRead(button1) == HIGH && P4 == 0)
  {
    if( paFlag == 0 )  
    {        
      pinActivated = millis();
    ...
    if( (millis() - pinActivated > buttonDelay) && (paFlag >= 1))
    {
  ...
  if( digitalRead(button1) == LOW && P4 >= 1 )
  {
    ...
    if( (millis() - pinActivated > buttonDelay) && (paFlag1 >=1))
    {
